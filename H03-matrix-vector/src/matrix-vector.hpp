...
public:
    ...
    {
        // TODO  Hint: See the same operator in the `Vector` class above.
        // Remember to map the two indices `i` and `j` to a single flat index.
    }
    T& operator()(I i, I j)
    {
        // TODO  Hint: Use Exactly the same implementation is the `const`
        // version.
    }
